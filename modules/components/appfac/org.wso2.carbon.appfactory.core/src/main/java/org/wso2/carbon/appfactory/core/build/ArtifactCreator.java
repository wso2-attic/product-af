...
public class ArtifactCreator extends AbstractAdmin {
	...
	public void createArtifact(String applicationId, String version, String revision, boolean doDeploy,
	                           String deployStage, String tagName) throws AppFactoryException {
		try {
			...
			if (appIsBuildable) {
				if (ServiceHolder.getContinuousIntegrationSystemDriver() == null) {
		  	    ...
		  	    ServiceHolder.getContinuousIntegrationSystemDriver().startBuild(applicationId, version, doDeploy,
					                                                                ...
					                                                                tenantUserName);
			    log.info("Start a build job [Buildable Artifact] in CI to Application ID : " + applicationId + " , version " + version + " by " + tenantDomain) ;
				...
				log.info("Start a generating artifact job in Appfactory NonBuildable artifact generator to Application ID : " + applicationId + " , version " + version + " by " + tenantDomain) ;
			}
