...
public class SCMClient {
			...
			try {
				...
				if (ArrayUtils.isNotEmpty(onlySubDirectries)) {
					for (File subDir : onlySubDirectries) {
						outcome = outcome && addRecursively(url, subDir.getAbsolutePath(), repository);
					}
				}
	 ...
	 * @throws RepositoryMgtException
	 */
	public boolean checkOut(String url, File checkOutDirectory, String revision, String version)
	                                                                                            throws RepositoryMgtException {
		if (AppFactoryConstants.TRUNK.equals(revision)) {
			revision = "";
		}
	...
	/**
	 * Add new files,delete removed files and checkin
	 * 
	 * @param url
	 * @param checkInDirectory
	 * @param msg
	...
	public boolean forceCheckIn(String url, File checkInDirectory, String msg, String targetVersion)
		...
		try {
			...
			checkInScmResult =
			                   scmManager.checkIn(repository, new ScmFileSet(checkInDirectory),
			                                      new ScmBranch(targetVersion), msg);
		} catch (ScmException e) {
			String message = "Error in executing checkIn operation on " + url;
			log.error(msg, e);
			throw new RepositoryMgtException(message, e);
