...
//        Checking whether the app directory exists. If not, the previous process has failed. Hence returning
    ...
    public static Application getApplicationInfo(String applicationId, String domainName) throws AppFactoryException {
        GenericArtifactImpl artifact = CommonUtil.getApplicationArtifact(applicationId, domainName);
	    return getAppInfoFromRXT(artifact);
    }

	/**
	 * Provides set of applications of the current tenant
	 *
	 * @param tenantDomain domain of the current tenant
	 * @return applications
	 * @throws AppFactoryException
	 */
	public static List<Application> getAllApplicationInfo(String tenantDomain) throws AppFactoryException {
		List<Application> applications = new ArrayList<Application>();
		try {
			GenericArtifactManager manager = CommonUtil.getApplicationRXTManager(tenantDomain);
			for (GenericArtifact artifact : manager.getAllGenericArtifacts()) {
				Application application = getAppInfoFromRXT(artifact);
				applications.add(application);
			}
		} catch (GovernanceException e) {
			String msg = "Error while getting applications for domain : " + tenantDomain;
			log.info(msg, e);
			throw new AppFactoryException(msg, e);
		}
	...
	/**
	 * This method retrieves the application info from the artifact
	 *
	 * @param artifact generic artifact
	 * @return Application
	 * @throws AppFactoryException
	 */
	public static Application getAppInfoFromRXT(GenericArtifact artifact) throws AppFactoryException {
        Application appInfo;

        try {
            appInfo =
                    new Application(artifact.getAttribute(AppFactoryConstants.RXT_KEY_APPINFO_KEY),
	        ...
	        String repoAccessability = artifact.getAttribute(AppFactoryConstants.RXT_KEY_APPINFO_REPO_ACCESSABILITY);
	        ...
	        appInfo = getAppInfoFromDao(appInfo);
	...
	/**
	 * This method retrieves application information from data layer
	 *
	 * @param appInfo Application object sends from the getAppInfoFromRXT
	 * @return appInfo
	 * @throws AppFactoryException
	 */
	private static Application getAppInfoFromDao(Application appInfo) throws AppFactoryException {
		try {
			appInfo.setBranchCount(applicationDAO.getBranchCount(appInfo.getId()));
			// set application creation status, if not found, consider as completed because previously
			// created applications does not contain this attribute
			String applicationCreationStatus = applicationDAO.getApplicationCreationStatus
					(appInfo.getId()).name();
			if (applicationCreationStatus != null) {
				appInfo.setApplicationCreationStatus(
						Constants.ApplicationCreationStatus.valueOf(applicationCreationStatus));
			} else {
				appInfo.setApplicationCreationStatus(Constants.ApplicationCreationStatus.COMPLETED);
			}
		}catch (AppFactoryException e){
			String errorMsg = String.format("Unable to extract information from database");
			log.error(errorMsg, e);
			throw new AppFactoryException(errorMsg, e);
		}
		return appInfo;
