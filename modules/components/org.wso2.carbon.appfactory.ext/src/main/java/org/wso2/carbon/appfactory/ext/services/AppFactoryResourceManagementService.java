 ...
 * is meant to be run on Greg.
 * </p>
 * 
 * <p>
 * Rational of having this service hosted on the Greg ( as oppose to use relevant Carbon Admin services to
	...
	private Registry getRegistry(String applicationId) throws AppFactoryException {
		...
		if (isAccessAllowed()) {
			...
			try {
				...
				registry = ServiceHolder.getInstance().getRegistryService().getGovernanceSystemRegistry(tenantId);
	...
	private boolean isAccessAllowed() throws AppFactoryException {
		...
		try {
			AuthorizationManager authorizationManager =
							ServiceHolder.getInstance().getRealmService()
