...
public class GitSCMClient {
    ...
    public boolean checkOut(String url, File checkOutDirectory)
        ...
        return processResult(checkOutScmResult);
    }
    
    public boolean checkIn(String url, File checkInDirectory, String msg , boolean isRemoveCheckInDirectory) throws AppFactoryException{
    	boolean result = checkIn( url,  checkInDirectory,  msg);
    	if(isRemoveCheckInDirectory){
    		removeDirectory(checkInDirectory);
    	}
    	return result ;
    }
    ...
    public boolean checkIn(String url, File checkInDirectory, String msg)
        ...
        return processResult(checkInScmResult);
    }
    
    private void removeDirectory(File workingDirectory) throws AppFactoryException{
    ...
    private boolean processResult(ScmResult result)
            throws AppFactoryException {
     ...
     * @throws AppFactoryException if add operation in SCM
     */
    public boolean add(String url, File currentFile) throws AppFactoryException {
        ...
        return processResult(addScmResult);
    }
    ...
    public boolean status(String url, File currentFile) throws AppFactoryException{
        ScmRepository repository = getRepository(url);
        ScmFileSet fileSet = new ScmFileSet(currentFile.getParentFile(),currentFile);
        StatusScmResult result;

        try{
            result = scmManager.status(repository,fileSet);
        } catch (ScmException e) {
            String errorMsg = "Unable to check status file from repository";
            log.error(errorMsg,e);
            throw new AppFactoryException(errorMsg,e);
        }
        return processResult(result);
