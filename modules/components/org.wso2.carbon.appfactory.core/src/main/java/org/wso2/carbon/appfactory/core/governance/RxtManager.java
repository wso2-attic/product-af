...
public class RxtManager {
    ...
    public List<Version> getRepoUserRxtForApplicationOfUser(String domainName, String applicationId, String userName)
        ...
        try {
            ...
            return getrepoUserRXTFromRegistry(userRegistry, applicationId, userName.split("@")[0], domainName);
        } catch (RegistryException e) {
    ...
    private List<Version> getrepoUserRXTFromRegistry(UserRegistry userRegistry, final String applicationId,
        ...
        GenericArtifactFilter artifactFilter = new GenericArtifactFilter() {
            ...
            public boolean matches(GenericArtifact artifact) throws GovernanceException {
                if (artifact != null && artifact.getAttribute("repouser_name") != null &&
                    artifact.getAttribute("repouser_key") != null) {
                    return artifact.getAttribute("repouser_name").equals(userName) &&
                           artifact.getAttribute("repouser_key").equals(applicationId);
                } else {
                    return false;
                }
            }
