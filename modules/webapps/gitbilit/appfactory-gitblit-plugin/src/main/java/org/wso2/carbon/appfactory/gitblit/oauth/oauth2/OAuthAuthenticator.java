...
public class OAuthAuthenticator {
    ...
    public OAuthAuthenticator(IStoredSettings settings) {
        ...
        String serverUrl = configuration.getProperty(GitBlitConstants.APPFACTORY_IS_URL, GitBlitConstants.APPFACTORY_IS_DEFAULT_URL);
        String serviceName = configuration.getProperty(GitBlitConstants.APPFACTORY_IS_OAUTH_SERVICE, GitBlitConstants.APPFACTORY_IS_OAUTH_DEFAULT_SERVICE);
        String adminuser = configuration.getProperty(GitBlitConstants.APPFACTORY_ADMIN_USERNAME, GitBlitConstants.APPFACTORY_ADMIN_USERNAME_DEFAULT_VALUE);
        String adminpass = configuration.getProperty(GitBlitConstants.APPFACTORY_ADMIN_PASSWORD, GitBlitConstants.APPFACTORY_ADMIN_PASSWORD_DEFAULT_VALUE);
        this.endPoint = serverUrl + "/" + serviceName;
        try {
			stub = new OAuth2TokenValidationServiceStub(endPoint);
			CarbonUtils.setBasicAccessSecurityHeaders(adminuser, adminpass, true, stub._getServiceClient());
		} catch (AxisFault e) {	
			logger.info("Stub initialization failed. " + e);
		}
	}

	public UserModel authenticate(String accessToken) {
        OAuth2TokenValidationRequestDTO  oauthReq = new OAuth2TokenValidationRequestDTO();
        oauthReq.setAccessToken(accessToken);
        oauthReq.setTokenType(BEARER_TOKEN_TYPE);
        OAuth2TokenValidationResponseDTO oauthResponse = null;
        try {
			oauthResponse =  stub.validate(oauthReq);			
		} catch (RemoteException e) {
			logger.info("Token Validation service call failed. " + e);
		}
        ...
        UserModel userModel = new AppFactoryGitBlitUserModel(username, this.configuration, new AppFactoryRepositoryAuthorizationClient(this.configuration));
		return userModel;
	}	
