...
do
    if [ "$c" = "--debug" ] || [ "$c" = "-debug" ] || [ "$c" = "debug" ]; then
          ...
          CMD="test"
    elif [ "$c" = "-Dprofile=cassandra" ] ; then
          CMD="cassandra"
    elif [ "$c" = "-Dprofile=zookeeper" ] ; then
          CMD="zookeeper"
    fi
done

if [ "$CMD" = "zookeeper" ]; then
echo Stating WSO2 MessageBroker - Profile ZooKeeper
...
do
    if [ "$CMD" = "cassandra" ]; then
        echo Stating WSO2 MessageBroker - Profile Cassandra
        $JAVACMD \
