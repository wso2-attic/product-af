...
# using nohup bash to avoid erros in solaris OS.TODO
  nohup bash $CARBON_HOME/bin/wso2server.sh >> $CARBON_HOME/repository/logs/nohup.out 2>&1 &
  exit 0
elif [ "$CMD" = "stop" ]; then
  export CARBON_HOME=$CARBON_HOME
...
export JENKINS_TENANT_HOME=$CARBON_HOME/repository/jenkins
...
do
    ...
    -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:+DisableExplicitGC -XX:CMSInitiatingOccupancyFraction=75 -XX:+UseCMSInitiatingOccupancyOnly -XX:+CMSClassUnloadingEnabled -XX:+CMSPermGenSweepingEnabled \
