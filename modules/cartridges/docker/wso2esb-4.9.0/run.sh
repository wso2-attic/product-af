...
startWkaMember() {
	...
	container_id=`docker run -e CONFIG_PARAM_CLUSTERING=true -d -P --name ${name} wso2/esb:4.9.0`
	memberId=$((memberId + 1))
...
startMember() {
	...
	container_id=`docker run -e CONFIG_PARAM_CLUSTERING=true -e CONFIG_PARAM_WKA_MEMBERS="[${wka_member_ip}:4100]" -d -P --name ${name} wso2/esb:4.9.0`
	memberId=$((memberId + 1))
